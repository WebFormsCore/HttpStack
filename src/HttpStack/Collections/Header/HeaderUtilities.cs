...
public static class HeaderUtilities
{
    ...
    public static NameValueHeaderValue? Find(IList<NameValueHeaderValue>? values, StringSegment name)
    {
        ...
        {
            var value = values[i];
            if (value.Name.Equals(nameAsString, StringComparison.OrdinalIgnoreCase))
            {
                return value;
            }
        }
    ...
    {
        var qualityParameter = Find(parameters, QualityName);
        if (value.HasValue)
        {
    ...
    private static int AdvanceCacheDirectiveIndex(int current, string headerValue)
    {
        ...
        {
            current++; // skip '='
            current += GetValueLength(headerValue, current);
        }
    ...
    public static bool TryParseNonNegativeInt32(StringSegment value, out int result)
    {
        ...
        return int.TryParse(
            ...
            value.Value
#endif
            , NumberStyles.None, NumberFormatInfo.InvariantInfo, out result);
    }
...
#else
        var sb = new StringBuilder(input.Length - backSlashCount);

        for (var i = 0; i < input.Length; i++)
        {
            int nextIndex = i + 1;
            if (nextIndex < input.Length && input[i] == '\\')
            {
                // If there is an backslash character as the last character in the string,
                // we will assume that it should be included literally in the unescaped string
                // Ex: "hello\\" => "hello\\"
...
#else
        var sb = new StringBuilder(input.Length + backSlashCount + 2);
