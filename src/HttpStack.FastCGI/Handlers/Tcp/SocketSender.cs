...
internal sealed class SocketSender : SocketAwaitableEventArgs
{
    ...
    public SocketSender() : this(PipeScheduler.ThreadPool)
    {
    }
...
#else
        SetBuffer(MemoryMarshal.TryGetArray(memory, out var segment) ? segment.Array : throw new InvalidOperationException(), 0, memory.Length);
#endif
    ...
    private void SetBufferList(in ReadOnlySequence<byte> buffer)
    {
        ...
        {
            _bufferList.Add(MemoryMarshal.TryGetArray(b, out var segment) ? segment : throw new InvalidOperationException());
        }
